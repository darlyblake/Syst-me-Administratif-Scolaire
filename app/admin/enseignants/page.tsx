...
export default function Page() {
  ...
  const load = useCallback(async () => { setLoading(true); setError(""); try { const establishments = await getAdminEstablishments(); setSchools(establishments); const results = await Promise.all(establishments.map(async school => { try { const page = await listStaffPaginated(school.id, 1, 100, "", true); return page.items.filter((staff: any) => staff.role === "enseignant" || staff.role === "teacher").map((staff: any) => ({ ...staff, establishment: school.name })) } catch { return [] } })); setTeachers(results.flat()) } catch (e) { setError(e instanceof Error ? e.message : "Impossible de charger les enseignants.") } finally { setLoading(false) } }, [])
