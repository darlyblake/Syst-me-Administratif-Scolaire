...
type Request = { id: string; establishment_id: string; student_id: string | null; document_type: string; message: string | null; response_message: string | null; delivery_method: string | null; response_document_id: string | null; status: string; created_at: string }
type Attachment = { id: string; request_id: string; file_name: string; mime_type: string; size_bytes: number; storage_path: string }
type ResponseDocument = { id: string; name: string; storage_path: string; mime_type: string | null }

const MAX_FILE_SIZE = 1024 * 1024
const ACCEPTED_TYPES = ["application/pdf", "image/jpeg", "image/png", "image/webp"]
const statusLabels: Record<string, string> = { pending: "En attente", in_progress: "En traitement", ready: "Disponible", rejected: "Refusée", cancelled: "Annulée", completed: "Terminée" }

export default function ParentDemandesPage() {
  const [links, setLinks] = useState<ChildLink[]>([])
  const [requests, setRequests] = useState<Request[]>([])
  const [attachments, setAttachments] = useState<Attachment[]>([])
  const [responseDocuments, setResponseDocuments] = useState<ResponseDocument[]>([])
  ...
  const load = async () => {
    ...
    const { data: guardianLinks, error: linksError } = await supabaseBrowser.from("student_guardians").select("student_id,establishment_id").eq("guardian_user_id", auth.user.id).eq("active", true)
    if (linksError) { setError(linksError.message); setLoading(false); return }
    const ids = (guardianLinks ?? []).map((l) => l.student_id)
    if (!ids.length) { setLinks([]); setRequests([]); setAttachments([]); setResponseDocuments([]); setLoading(false); return }
    const { data: students, error: studentsError } = await supabaseBrowser.from("students").select("id,first_name,last_name,student_number,establishment_id").in("id", ids).order("last_name")
    if (studentsError) { setError(studentsError.message); setLoading(false); return }
    const studentMap = new Map((students ?? []).map((s) => [s.id, s]))
    const safeLinks = (guardianLinks ?? []).flatMap((l) => { const s = studentMap.get(l.student_id); return s && s.establishment_id === l.establishment_id ? [{ student_id: s.id, establishment_id: l.establishment_id, student_name: `${s.first_name} ${s.last_name}`, student_number: s.student_number }] : [] })
    setLinks(safeLinks)
    const establishmentIds = [...new Set(safeLinks.map((l) => l.establishment_id))]
    ...
    if (establishmentIds.length) {
      const { data: establishmentData } = await supabaseBrowser.from("establishments").select("id,name").in("id", establishmentIds)
      setEstablishments(Object.fromEntries((establishmentData ?? []).map((e) => [e.id, e.name])))
    }
    const { data: requestData, error: requestError } = await supabaseBrowser.from("parent_document_requests").select("id,establishment_id,student_id,document_type,message,response_message,delivery_method,response_document_id,status,created_at").eq("parent_user_id", auth.user.id).order("created_at", { ascending: false }).limit(50)
    if (requestError) { setError(requestError.message); setLoading(false); return }
    const safeRequests = (requestData ?? []).filter((r) => safeLinks.some((l) => l.establishment_id === r.establishment_id && (!r.student_id || l.student_id === r.student_id))) as Request[]
    setRequests(safeRequests)
    const requestIds = safeRequests.map((r) => r.id)
    if (requestIds.length) {
      const { data: attachmentData, error: attachmentError } = await supabaseBrowser.from("parent_document_request_attachments").select("id,request_id,file_name,mime_type,size_bytes,storage_path").in("request_id", requestIds).order("created_at")
      if (attachmentError) { setError(attachmentError.message); setLoading(false); return }
      setAttachments((attachmentData ?? []) as Attachment[])
      const responseIds = safeRequests.map((r) => r.response_document_id).filter(Boolean) as string[]
      if (responseIds.length) {
        const { data: docs } = await supabaseBrowser.from("documents").select("id,name,storage_path,mime_type").in("id", responseIds)
        setResponseDocuments((docs ?? []) as ResponseDocument[])
      } else setResponseDocuments([])
    } else { setAttachments([]); setResponseDocuments([]) }
    setLoading(false)
  }

  useEffect(() => { void load() }, [])

  const onEstablishmentChange = (id: string) => { setEstablishmentId(id); const first = links.find((l) => l.establishment_id === id); setStudentId(first?.student_id ?? "") }
  const addFiles = (incoming: FileList | null) => {
    if (!incoming) return
    setError(""); const next = [...files]
  ...
  const submit = async () => {
    ...
    try {
      const { data, error: rpcError } = await supabaseBrowser.rpc("parent_create_document_request", { p_establishment_id: establishmentId, p_document_type: documentType.trim(), p_student_id: studentId || null, p_message: message.trim() || null })
      if (rpcError) throw rpcError
      const requestId = data as string
      const { data: auth } = await supabaseBrowser.auth.getUser(); if (!auth.user) throw new Error("Session parent introuvable.")
      for (const file of files) {
        const storagePath = `${auth.user.id}/${requestId}/${crypto.randomUUID()}-${file.name.replace(/[^a-zA-Z0-9._-]/g, "_")}`
        const { error: uploadError } = await supabaseBrowser.storage.from("parent-documents").upload(storagePath, file, { contentType: file.type, upsert: false }); if (uploadError) throw uploadError
        const { error: attachmentError } = await supabaseBrowser.from("parent_document_request_attachments").insert({ request_id: requestId, parent_user_id: auth.user.id, storage_path: storagePath, file_name: file.name, mime_type: file.type, size_bytes: file.size })
        if (attachmentError) { await supabaseBrowser.storage.from("parent-documents").remove([storagePath]); throw attachmentError }
      }
      setMessage(""); setFiles([]); setSuccess("Votre demande a été envoyée à l’établissement."); await load()
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Impossible d'envoyer la demande.") } finally { setSending(false) }
  }
