...
import { GraduationCap, Phone, Mail, CreditCard, FileText, UserX, RefreshCw } from "lucide-react"
import { Card, CardContent, CardHeader } from "@/components/ui/card"
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"
import { useParentPortal } from "@/hooks/use-parent-portal"

function formatDate(value: string | null) { return value ? new Date(value).toLocaleDateString("fr-FR",{day:"numeric",month:"long",year:"numeric"}) : "—" }

export default function ParentsEnfantsPage() {
  const { loading, error, refresh, children, grades, payments } = useParentPortal()
  if (loading) return <div className="flex min-h-[50vh] items-center justify-center text-pierre">Chargement...</div>

  return <div className="space-y-6">
    <div className="flex flex-wrap justify-between gap-3"><div><h1 className="text-2xl font-bold text-terre">Mes enfants</h1><p className="text-pierre">{children.length} enfant{children.length>1?"s":""} associé{children.length>1?"s":""} à votre compte</p></div><Button variant="outline" onClick={()=>void refresh()}><RefreshCw className="mr-2 h-4 w-4"/>Actualiser</Button></div>
    {error && <Card className="border-red-200 bg-red-50"><CardContent className="p-4 text-red-700">{error}</CardContent></Card>}
    {children.length===0 && !error && <Card><CardContent className="py-10 text-center text-pierre">Aucun enfant n'est associé à votre compte.</CardContent></Card>}
    <div className="grid gap-6 lg:grid-cols-2">{children.map((child)=>{
      const childGrades=grades.filter(g=>g.student_id===child.id)
      const average=childGrades.length?childGrades.reduce((s,g)=>s+g.score,0)/childGrades.length:null
      const paid=payments.filter(p=>p.enrollment_id===child.enrollment_id).reduce((s,p)=>s+p.amount,0)
      return <Card key={child.id} className="overflow-hidden border-terre/10">
