...
export function ParentNav() {
  ...
  const notifs = serviceParents.compterNotificationsNonLues()
  const msgs = serviceParents.compterMessagesNonLus()

  const NavLinks = ({ onNavigate }: { onNavigate?: () => void }) => (
    <nav className="flex flex-col gap-1">
      {liens.map(({ href, label, icon: Icon, badge }) => {
        const actif = pathname === href || pathname?.startsWith(href + "/")
        const count = badge === "notifs" ? notifs : badge === "msgs" ? msgs : 0
        return (
          <Link
            key={href}
            href={href}
            ...
            className={cn(
              ...
              actif
                ? "bg-terre text-white shadow-soft"
                : "text-pierre hover:bg-terre-soft hover:text-terre",
            )}
          >
  ...
  return (
    <>
      ...
      <div className="sticky top-0 z-30 flex items-center justify-between border-b border-terre/10 bg-papier/95 px-4 py-3 backdrop-blur lg:hidden">
        <div>
          <p className="text-sm font-bold text-terre">Espace Parents</p>
          <p className="text-xs text-pierre">{utilisateur?.nomUtilisateur ?? "Parent"}</p>
        </div>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 bg-encre/30 lg:hidden" onClick={() => setMobileOpen(false)}>
          <div
            className="absolute left-0 top-0 flex h-full w-72 flex-col bg-papier p-4 shadow-soft"
            onClick={(e) => e.stopPropagation()}
          >
            <p className="mb-4 text-lg font-bold text-terre">Menu</p>
            <NavLinks onNavigate={() => setMobileOpen(false)} />
            <div className="mt-auto pt-4">
              <Button variant="outline" className="w-full rounded-xl" onClick={() => deconnecter()}>
      ...
      <aside className="fixed left-0 top-0 z-20 hidden h-screen w-64 flex-col border-r border-terre/10 bg-papier px-4 py-6 lg:flex">
        <div className="mb-8 px-2">
          <p className="text-xs font-semibold uppercase tracking-wide text-terre">Scolarité</p>
          <h1 className="text-xl font-bold text-terre">Espace Parents</h1>
          <p className="mt-1 truncate text-sm text-pierre">
            {utilisateur?.nomUtilisateur ?? "Parent connecté"}
          </p>
        </div>
        <div className="flex-1 overflow-y-auto">
          <NavLinks />
        </div>
        <Button variant="outline" className="mt-4 w-full rounded-xl" onClick={() => deconnecter()}>
          <LogOut className="mr-2 h-4 w-4" />
          Déconnexion
        </Button>
      </aside>
