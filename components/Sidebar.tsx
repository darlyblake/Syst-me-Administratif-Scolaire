...
export default function Sidebar({ isOpen, setIsOpen }: SidebarProps) {
 const pathname=usePathname(); const router=useRouter(); const [isLoggingOut,setIsLoggingOut]=useState(false)
 ...
 const handleLogout=async()=>{
   ...
   try {
     // Supprime la session locale et invalide le refresh token Supabase.
     const { error } = await supabaseBrowser.auth.signOut({ scope: "local" })
     if(error) throw error
     // Nettoyage des données UI non sensibles qui pourraient afficher un ancien contexte.
     ...
     // Même en cas d'erreur réseau, on évite de laisser l'utilisateur bloqué sur une session ambiguë.
     router.replace("/connexion")
     router.refresh()
