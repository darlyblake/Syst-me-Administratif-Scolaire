...
const ROUTE_PERMISSIONS: Array<[string, string]> = [
  ...
  ["/ecole/enseignants", "staff.view"], ["/ecole/personnel", "staff.view"], ["/ecole/etat-salaire", "staff.view"],
  ["/ecole/heures-vacataires", "staff.view"], ["/ecole/payments", "payments.view"], ["/ecole/comptabilite", "finance.view"],
  ["/ecole/archivage", "documents.view"], ["/ecole/documents", "documents.view"], ["/ecole/communication", "communication.view"],
  ["/ecole/evenements", "events.view"], ["/ecole/service-technique", "support.view"], ["/ecole/settings", "settings.view"],
]
...
export function PermissionRoute({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!utilisateur || !isSchoolAccount || !etablissementActif?.id) {
      router.replace("/connexion")
      return
    }
    if (!allowed) router.replace("/ecole/tableau-bord")
  }, [allowed, etablissementActif?.id, estEnCoursDeChargement, isSchoolAccount, router, utilisateur])

  if (estEnCoursDeChargement || !utilisateur) return <div className="min-h-[40vh] flex items-center justify-center text-sm text-pierre">Vérification de la session…</div>
  if (!allowed) return <div className="min-h-[40vh] flex items-center justify-center text-sm text-pierre">Accès non autorisé.</div>
  return <>{children}</>
}
