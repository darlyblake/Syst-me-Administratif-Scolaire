...
export function LoginPage() {
  ...
  const openBook = (nextView: "login" | "register") => {
    ...
    setLoginError(""); setRegisterMessage(""); setForgotError(""); setForgotMessage(""); setTurning(true)
  ...
  const changeOpenPage = (nextView: "login" | "forgot" | "register") => {
    if (turning) return
    if (nextView === "forgot") {
      router.push(`/auth/mot-de-passe-oublie?espace=${encodeURIComponent(espace)}`)
      return
    }
    setTurning(true)
    window.setTimeout(() => { setView(nextView); setTurning(false) }, 520)
  ...
  const closeBook = () => {
    if (turning) return
    setTurning(true)
    window.setTimeout(() => { setLoginError(""); setForgotError(""); setForgotMessage(""); setRegisterMessage(""); setView("closed"); setTurning(false) }, 520)
  }
  ...
  const handleForgotSubmit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); setForgotError(""); setForgotMessage("")
    ...
    router.push(`/auth/mot-de-passe-oublie?espace=${encodeURIComponent(espace)}&email=${encodeURIComponent(email)}`)
  }
    ...
    <main className={styles.shell}>
      ...
      <div className={`${styles.stage} ${introFinished ? styles.stageReady : styles.stageApproaching} ${bookOpen ? styles.stageOpen : ""} ${turning ? styles.stageTurning : ""}`}>
        <div className={`${styles.book} ${bookOpen ? styles.bookOpen : styles.bookClosed}`}>
          ...
          <aside className={styles.leftPage} aria-label="Guide de l'application"><div className={styles.pageContent}><span className={styles.pageNumber}>Page {guideNumber}</span><span className={styles.eyebrow}>{view === "forgot" ? "Guide récupération" : view === "register" ? "Guide inscription" : "Administration scolaire"}</span><h1>{view === "forgot" ? "Retrouver votre accès" : view === "register" ? "Créer votre espace" : "Système de Gestion Scolaire"}</h1><p className={styles.intro}>{view === "forgot" ? "Quelques étapes simples permettent de récupérer l’accès à votre compte en toute sécurité." : view === "register" ? "Créez votre espace pour accéder aux outils de gestion et accompagner efficacement votre établissement." : "Un espace unique pour organiser simplement la vie administrative et pédagogique de votre établissement."}</p><div className={styles.features}>{view === "forgot" ? <><span>Indiquer votre email</span><span>Recevoir les instructions</span><span>Vérifier votre boîte mail</span><span>Choisir un nouveau mot de passe</span></> : view === "register" ? <><span>Renseigner vos informations</span><span>Créer votre accès</span><span>Sécuriser votre compte</span><span>Accéder à votre espace</span></> : <><span>Élèves</span><span>Enseignants</span><span>Classes</span><span>Notes</span><span>Absences</span><span>Finances</span><span>Communication</span></>}</div></div></aside>
          <section className={styles.rightPage} aria-label={view === "register" ? "Formulaire d'inscription" : view === "forgot" ? "Récupération du mot de passe" : "Formulaire de connexion"} aria-hidden={!bookOpen}><div className={styles.pageContent}><span className={styles.pageNumber}>Page {pageNumber}</span>
            {view === "login" ? <><span className={styles.eyebrow}>Accès sécurisé</span><h2>{espaceLabels[espace]}</h2><p className={styles.formIntro}>Connectez-vous pour accéder à votre espace.</p><form onSubmit={handleLoginSubmit} className={styles.form} noValidate><label>Adresse email<input type="email" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} placeholder="votre@ecole.fr" autoComplete="email" /></label><label>Mot de passe<span className={styles.password}><input type={showPassword ? "text" : "password"} value={loginPassword} onChange={e => setLoginPassword(e.target.value)} placeholder="••••••••" autoComplete="current-password" /><button type="button" onClick={() => setShowPassword(v => !v)} aria-label="Afficher ou masquer le mot de passe">{showPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></span></label>{loginError && <p className={styles.error} role="alert">{loginError}</p>}<button type="submit" className={styles.primaryButton} disabled={loading}>{loading ? <><Loader2 className={styles.smallSpin} /> Connexion...</> : <><LogIn size={17} /> Se connecter</>}</button><button type="button" className={styles.textButton} onClick={() => changeOpenPage("forgot")}>Mot de passe oublié ?</button></form></>
            : view === "forgot" ? <><span className={styles.eyebrow}>Récupération sécurisée</span><h2>Mot de passe oublié</h2><p className={styles.formIntro}>Saisissez l’adresse email associée à votre compte pour recevoir les instructions de récupération.</p><form onSubmit={handleForgotSubmit} className={styles.form} noValidate><label>Adresse email<input type="email" value={forgotEmail} onChange={e => setForgotEmail(e.target.value)} placeholder="votre@ecole.fr" autoComplete="email" /></label>{forgotError && <p className={styles.error} role="alert">{forgotError}</p>}{forgotMessage && <p className={styles.notice} role="status">{forgotMessage}</p>}<button type="submit" className={styles.primaryButton}><KeyRound size={17} /> Envoyer les instructions</button><button type="button" className={styles.textButton} onClick={() => changeOpenPage("login")}>Retour à la connexion</button></form></>
