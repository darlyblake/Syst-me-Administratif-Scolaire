...
interface AssignmentRow {
  ...
  school_classes?: { name: string | null } | null
...
export async function obtenirEnseignantsSupabase(etablissementId: string): Promise<DonneesEnseignant[]> {
  ...
  const { data: assignmentData, error: assignmentError } = await supabaseBrowser
    .from("class_subjects")
    .select("class_id,subject_id,teacher_id,school_classes(name),subjects(name)")
    .in("teacher_id", ids)
...
export async function creerEnseignantSupabase(
  etablissementId: string,
  input: Omit<DonneesEnseignant, "id" | "identifiant" | "motDePasse">,
): Promise<DonneesEnseignant> {
...
export async function modifierEnseignantSupabase(
  id: string,
  changes: Partial<DonneesEnseignant>,
): Promise<boolean> {
