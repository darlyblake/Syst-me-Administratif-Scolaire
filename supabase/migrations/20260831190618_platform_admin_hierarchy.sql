create table if not exists public.platform_admins (
  ...
  constraint platform_admin_permissions_allowed check (
    permissions <@ array[
      'dashboard.view','establishments.view','establishments.manage',
      'subscriptions.manage','users.view','admins.manage','support.manage','settings.manage'
    ]::text[]
  )
);
...
select p.id, null, array[
  'dashboard.view','establishments.view','establishments.manage','subscriptions.manage',
  'users.view','admins.manage','support.manage','settings.manage'
]::text[], true, true, p.created_at, now()
from public.profiles p
where p.account_type = 'platform_admin'
  and p.id = (select p2.id from public.profiles p2 where p2.account_type='platform_admin' order by p2.created_at asc, p2.id asc limit 1)
...
from public.profiles p
cross join lateral (select user_id, permissions from public.platform_admins where is_root order by created_at asc limit 1) r
where p.account_type='platform_admin'
...
begin
  ...
  return query select pa.user_id,u.email::text,p.first_name,p.last_name,pa.active,pa.is_root,pa.created_by,concat_ws(' ',cp.first_name,cp.last_name),pa.permissions,pa.created_at
  from public.platform_admins pa join auth.users u on u.id=pa.user_id left join public.profiles p on p.id=pa.user_id left join public.profiles cp on cp.id=pa.created_by
  order by pa.is_root desc,pa.created_at asc;
end; $$;
